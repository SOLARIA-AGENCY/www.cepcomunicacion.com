...
export const trackFAQCreator: FieldHook = async ({
  data,
  req,
  operation,
  value,
  ...
  if (operation === 'create') {
    if (!req.user) {
      throw new Error('Authentication required to create FAQs');
    }
    return req.user.id;
  }
...
export default trackFAQCreator;
